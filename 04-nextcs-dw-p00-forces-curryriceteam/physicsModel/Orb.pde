  class Orb {
    ...
    //movement behavior
      ...
      PVector getMagneticForce(PVector magnet, float magneticStrength) {
      PVector force = PVector.sub(magnet, center);
      float distance = constrain(force.mag(), 10, 300); // Avoid extreme values
      force.normalize();
      float strength = magneticStrength / (distance * distance); // Inverse square law
      force.mult(strength);
      return force;
    }
